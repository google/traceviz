...
describe('data query test', () => {
  ...
  // canned data.
  const tdq = new TestDataQuery((err) => fail(err));
  ...
  let wantRequest: Request | undefined;
  const reqSub = GLOBAL_TEST_DATA_FETCHER.requestChannel.subscribe((req) => {
    if (wantRequest !== undefined) {
      expect(req).toEqual(wantRequest);
    }
  });
  afterAll(() => {
    reqSub.unsubscribe();
  ...
  it('issues a request with a single DataSeriesRequest, expecting success',
    (() => {
      ...
      GLOBAL_TEST_DATA_FETCHER.responseChannel.next(buildEmptyResponse(seriesReq1));
      var response: ResponseNode | undefined;
      tdq.fetchDataSeries(seriesReq1, (r) => { response = r }, () => fail('unexpected cancellation'));
  ...
  it('deduplicates multiple DataSeriesRequests with the same series name',
    (() => {
      ...
      GLOBAL_TEST_DATA_FETCHER.responseChannel.next(buildEmptyResponse(seriesReq2));
      var response: ResponseNode | undefined;
      tdq.fetchDataSeries(seriesReq1, (r) => { response = r }, () => fail('unexpected cancellation'));
      expect(response).toBeUndefined();
      tdq.triggerUpdates();
  ...
  it('issues a request with two DataSeriesRequests, expecting success', (() => {
    ...
    GLOBAL_TEST_DATA_FETCHER.requestChannel.next(dataRequest(seriesReq1, seriesReq2));
    GLOBAL_TEST_DATA_FETCHER.responseChannel.next(buildEmptyResponse(seriesReq1, seriesReq2));
    var response1: ResponseNode | undefined;
    var response2: ResponseNode | undefined;
    tdq.fetchDataSeries(seriesReq1, (r) => { response1 = r }, () => fail('unexpected cancellation'));
    tdq.fetchDataSeries(seriesReq2, (r) => { response2 = r }, () => fail('unexpected cancellation'));
  ...
  it('Cancels two DataSeriesRequests', (() => {
    ...
    let responses = 0;
    let cancellations = 0;
    tdq.fetchDataSeries(seriesReq1, (r) => responses++, () => cancellations++);
    tdq.fetchDataSeries(seriesReq2, (r) => responses++, () => cancellations++);
    tdq.triggerUpdates();
