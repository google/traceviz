...
function sec(sec: number): Timestamp {
  ...
  function prettyPrint(chart: XYChart): string {
    ...
    ret.push(`X Domain: [${prettyPrintPointValue(chart.xAxis.min)}, ${
        prettyPrintPointValue(
            chart.xAxis.max)}] (${chart.xAxis.category.displayName})`);
    ret.push(`Y Domain: [${prettyPrintPointValue(chart.yAxis.min)}, ${
        prettyPrintPointValue(
    ...
    for (const series of chart.series) {
      ...
      ret.push(`  Category ${cat.id}: '${cat.displayName}' (${cat.description}, ${
          cols.primary})`);
  ...
  describe('xy chart test', () => {
    it('gets xy chart', () => {
      ...
      expect(prettyPrint(chart))
          .toEqual(`X Domain: [${sec(0).toDate().toString()}, ${
              sec(100).toDate().toString()}] (time from start)
