...
export function compress(obj: object): string {
  ...
  const compressedStr =
      Array.from(compressedBytes).map((b) => String.fromCharCode(b)).join('');
...
export function decompress<T = never>(compressed: string): T|undefined {
  ...
  const compressedBytes =
      Uint8Array.from(compressedStr.split('').map((c) => c.charCodeAt(0)));
 ...
 */
export function unserializeHashFragment(hashFragment: string):
    {[k: string]: string} {
...
export function serializeHashFragment(hashMap: {[k: string]: string}): string {
  ...
  return !keys.length ?
      ...
      `#${
          keys.map(
                  (k) =>
                      hashMap[k] ? `${k}=${encodeURIComponent(hashMap[k])}` : k)
              .join('&')}`;
}
