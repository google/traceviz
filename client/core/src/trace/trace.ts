...
import {
  ...
  TimestampAxis,
  unionAxes
} from '../continuous_axis/continuous_axis.js';
...
enum Keys {
  ...
  SPAN_WIDTH_CAT_PX = 'trace_span_width_cat_px',
  SPAN_PADDING_CAT_PX = 'trace_span_padding_cat_px',
  CATEGORY_HEADER_CAT_PX = 'trace_category_header_cat_px',
  CATEGORY_PADDING_CAT_PX = 'trace_category_padding_cat_px',
  CATEGORY_MARGIN_TEMP_PX = 'trace_category_margin_temp_px',
  CATEGORY_MIN_WIDTH_CAT_PX = 'trace_category_min_width_cat_px',
  CATEGORY_BASE_WIDTH_TEMP_PX = 'trace_category_base_width_temp_px',
}
 ...
 */
export const PAYLOAD_KEYS = [Keys.NODE_TYPE, Keys.PAYLOAD_TYPE];
...
export class Span {
  ...
  static fromNode(node: ResponseNode): Span {
    ...
    return new Span(
      offset, duration, children, subspans, payloads, properties, height);
  }
}
    ...
    for (const child of node.children) {
      switch (child.properties.expectNumber(Keys.NODE_TYPE)) {
        ...
        default:
          throw new ConfigurationError(
            `unsupported node type ${child.properties.expectNumber(
              Keys.NODE_TYPE)} as Subspan child`)
            .from(SOURCE)
            .at(Severity.ERROR);
      }
    }
    return new Subspan(offset, duration, payloads, properties);
  }
}
