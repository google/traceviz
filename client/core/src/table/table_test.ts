...
import { CanonicalTable, TableCell, TableHeader, TableRow } from "./table.js";
...
it('gets render properties', () => {
    const table = new CanonicalTable(node(
        valueMap(
    ...
    it('gets rows and columns', () => {
      ...
      const cs = [
        new TableHeader(nameColumn),
        new TableHeader(colorColumn),
        new TableHeader(flavorColumn),
        new TableHeader(labelColumn),
      ];
      expect(table.rowSlice()).toEqual([
        new TableRow(vanillaRow, cs), new TableRow(chocolateRow, cs),
        new TableRow(strawberryRow, cs)
      ]);
      const thisRow = new TableRow(strawberryRow, cs);
      expect(thisRow.cells(table.columns()).map((cell: TableCell) => cell.value))
          .toEqual([
            str('strawberry'), str('pink'), str('berrylicious'),
            str('strawberry (pink)')
          ]);
      expect(thisRow.cells(table.columns('label', 'flavor'))
                 .map((cell: TableCell) => cell.value))
          .toEqual([str('strawberry (pink)'), str('berrylicious')]);
      // Expect to find the row payload in vanilla.
      expect(table.rowSlice()
                 .map(
                     row => new Array(...row.payloadsByType.values())
                                .filter(
                                    (payloadNode: ResponseNode) =>
                                        payloadNode.properties.has('label'))
                                .map(
                                    (payloadNode: ResponseNode) =>
                                        payloadNode.properties.expectString(
                                            'label')))
                 .reduce((prev, next) => {
                   prev.push(...next);
                   return prev;
                 }, new Array<string>()))
          .toEqual(['Vanilla is derived from the seed pod of an orchid!']);
    });
  });
