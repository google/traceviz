...
import { DoubleValue, DurationValue, fromV, IntegerListValue, IntegerValue, StringListValue, StringValue, TimestampValue } from './value.js';
...
export class ValueMap {
    map;
    constructor(props = new Map([]), stringTable = []) {
        let map = new Map();
        if (props instanceof Map) {
            ...
            for (const kv of props) {
                ...
                if (value === undefined) {
                    throw new ConfigurationError(`value with key '${key}' can't be parsed`)
                        .from(SOURCE)
                        .at(Severity.ERROR);
                }
                map.set(key, value);
    ...
    updateFromExportedKeyValueMap(update) {
        for (const [key, exportedValue] of Object.entries(update)) {
            if (!this.has(key)) {
                throw new ConfigurationError(`can't update ValueMap from JSON: missing key ${key}`)
                    .from(SOURCE)
                    .at(Severity.ERROR);
            }
    ...
    format(fmtString) {
        ...
        while (true) {
            ...
            if (!matches[1] && !matches[2]) {
                throw new ConfigurationError(`format string '${fmtString}' is ill-formed`)
                    .from(SOURCE)
                    .at(Severity.ERROR);
            }
            ...
            if (matches[2] === '$$') {
                ...
                if (val === undefined) {
                    throw new ConfigurationError(`required property '${propName}' is not present in Datum`)
                        .from(SOURCE)
                        .at(Severity.ERROR);
                }
                else {
    ...
    with(...entries) {
        ...
        return new ValueMap(new Map(combinedMapEntries));
    }
    /**
        ...
        for (let idx = 1; idx < vms.length; idx++) {
            for (const [key, value] of vms[idx].map) {
                ...
                if (existingVal !== undefined && existingVal.compare(value) !== 0) {
                    throw new ConfigurationError(`can't union ValueMaps: key ${key} maps to different values ${value} and ${existingVal}`)
                        .from(SOURCE)
                        .at(Severity.ERROR);
                }
...
//# sourceMappingURL=value_map.js.map