...
describe('value map test', () => {
    ...
    it('constructs from raw map', () => {
        const mapMap = new ValueMap(new Map([['key', int(3)]]));
        expect([...mapMap.entries()]).toEqual([['key', int(3)]]);
    });
    it('formats', () => {
        const vm = new ValueMap(new Map([
            ['title', str('Alice\'s Adventures in Wonderland')],
            ['type', str('book')], ['author', str('Lewis Carroll')],
            ['publication_year', int(1865)]
        ]));
        const fmtStr = `Let's look at '$(title)', a $(type) published by $(author) in $(publication_year).`;
        expect(vm.format(fmtStr))
            .toEqual(`Let's look at 'Alice's Adventures in Wonderland', a book published by Lewis Carroll in 1865.`);
    });
    it('rejects bad formats', () => {
        for (const badFmt of [`a bare $`, `paren $(problems`, `missing $(variable)`]) {
            expect(() => {
    ...
    it('serializes to JSON and updates from JSON', () => {
        ...
        expect(vm.exportKeyValueMap())
            .toEqual({ 'str': 'a', 'int': 1, 'strs': ['x', 'y', 'z'], 'ints': [7, 8, 9] });
        // Expect working unserialization
        const vm2 = new ValueMap(new Map([
            ['str', str('b')],
            ['int', int(3)],
            ['strs', strs('l', 'm', 'n', 'o', 'p')],
    ...
    it('removes properties via without()', () => {
        expect((new ValueMap(new Map([
            ['str', str('b')],
            ['int', int(3)],
            ['strs', strs('l', 'm', 'n', 'o', 'p')],
            ['ints', ints(1000, 2000, 3000)],
        ]))).without('str', 'strs', 'foos'))
            .toEqual(new ValueMap(new Map([
            ['int', int(3)],
            ['ints', ints(1000, 2000, 3000)],
        ])));
    });
    it('watches properly', () => {
        const i = int(1);
        const s = str('hello');
        const vm = valueMap({ key: 'foo', val: i }, { key: 'bar', val: s });
        const obs = vm.watch();
        let updates = 0;
    ...
    it('unions properly', () => {
        expect(ValueMap.union(valueMap({ key: 'foo', val: int(1) }, { key: 'bar', val: str('hello') }), valueMap({ key: 'foo', val: int(1) }, { key: 'baz', val: ints(1, 2, 3) }), valueMap({ key: 'baz', val: ints(1, 2, 3) }, { key: 'bar', val: str('hello') })))
            .toEqual(valueMap({ key: 'foo', val: int(1) }, { key: 'bar', val: str('hello') }, { key: 'baz', val: ints(1, 2, 3) }));
        expect(() => {
            ValueMap.union(valueMap({ key: 'foo', val: int(1) }, { key: 'bar', val: str('hello') }), valueMap({ key: 'foo', val: int(2) }, { key: 'baz', val: ints(1, 2, 3) }));
        }).toThrow();
    });
});
//# sourceMappingURL=value_map_test.js.map