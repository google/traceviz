...
describe('interactions test', () => {
    ...
    it('sets local values', () => {
        const ids = strs('a', 'b');
        const vm = valueMap({ key: 'ids', val: ids }, { key: 'other_ids', val: strs('c', 'd') });
    ...
    it('toggles local values', () => {
        ...
        const vm = valueMap({ key: 'ids', val: ids }, { key: 'other_ids', val: strSet('b', 'c') });
    ...
    it('toggles-or-sets local values', () => {
        const id = int(3);
        const vm = valueMap({ key: 'id', val: id }, { key: 'other_id', val: int(3) });
    ...
    it('sets-if-empty local values', () => {
        const ids = strs();
        const vm = valueMap({ key: 'ids', val: ids }, { key: 'other_ids', val: strs('a', 'b') }, { key: 'more_ids', val: strs('c', 'd') });
    ...
    it('handles complex predicate', () => {
        ...
        const weight = int(0);
        const vm = valueMap({ key: 'weight', val: weight });
        const tickerTape = [];
        boundsChecker.match()(vm).subscribe((m) => {
    ...
    it('conditionally executes with if', () => {
        ...
        const cond = new If(new Equals(valWrap, new FixedValue(int(0))), new SetP(numberTextWrap, new FixedValue(str('none'))), new If(new Equals(valWrap, new FixedValue(int(1))), new SetP(numberTextWrap, new FixedValue(str('one'))), new SetP(numberTextWrap, new FixedValue(str('several')))));
    ...
    it('conditionally executes with switch', () => {
        ...
        const cond = new Switch([
            new Case(new Equals(valWrap, new FixedValue(int(0))), [new SetP(numberTextWrap, new FixedValue(str('none')))]),
            new Case(new Equals(valWrap, new FixedValue(int(1))), [new SetP(numberTextWrap, new FixedValue(str('one')))]),
            new Case(new True(), [new SetP(numberTextWrap, new FixedValue(str('several')))]),
        ]);
        expect(numberText.val).toEqual('');
        cond.update();
        expect(numberText.val).toEqual('none');
    ...
    it('watches', () => {
        ...
        const vm = valueMap({ key: 'highlightedStartOffset', val: highlightedStartOffset }, { key: 'highlightedEndOffset', val: highlightedEndOffset });
        const w = new Watch('highlight range', vm);
        const tickerTape = [];
        const unsub = new Subject();
        w.watch((vm) => {
            tickerTape.push(Array.from(vm.keys()).map((key) => `${key}: ${vm.get(key).toString()}`));
        }, unsub);
    ...
    it('interacts', () => {
        ...
        class Bump extends Update {
            ...
            update(localState) {
                const val = this.vr.get(localState);
                val.val = val.val + 1;
            }
            get autoDocument() {
        ...
        const bumpWeightOnClick = new Action('weight', 'click', [new Bump(weightRef)])
            .withHelpText('Upon \'click\' on a \'weight\', bumps that \'weight\'.', 
        ...
        const interactions = new Interactions()
            ...
            .withWatch(new Watch('highlight range', valueMap({ key: 'start', val: start }, { key: 'end', val: end })));
  ...
  Trigger 'highlight range' on changes to arguments (Watch)`);
        ...
        const weight = int(0);
        const vm = valueMap({ key: 'label', val: label }, { key: 'weight', val: weight });
        const tickerTape = [];
        interactions.match('graph', 'show info')(vm).subscribe((m) => {
            tickerTape.push(`${weight.val}: ${m}`);
        });
        for (let i = 0; i < 12; i++) {
...
//# sourceMappingURL=interactions_test.js.map