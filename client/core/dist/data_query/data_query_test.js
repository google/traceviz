...
function buildEmptyResponse(...seriesRequests) {
...
describe('data query test', () => {
    ...
    // canned data.
    const tdq = new DataQuery((err) => {
        fail(err);
    });
    // Set up global filters, and provide them to the TestDataQuery
    const gf1 = int(1);
    const gf2 = str('hello');
    const globalFilters = valueMap({ key: 'count', val: gf1 }, { key: 'greetings', val: gf2 });
    // A helper to build DataRequests.
    function dataRequest(...seriesRequests) {
        return {
    ...
    it('issues a request with a single DataSeriesRequest, expecting success', (() => {
        ...
        let response;
        tdq.fetchDataSeries(seriesReq1, (r) => {
    ...
    it('issues a request with two DataSeriesRequests, expecting success', (() => {
        ...
        tdq.fetchDataSeries(seriesReq2, (r) => {
            response2 = r;
        }, () => {
            fail('unexpected cancellation');
        });
...
//# sourceMappingURL=data_query_test.js.map