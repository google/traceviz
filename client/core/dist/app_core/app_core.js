...
export class AppCore {
    ...
    configurationErrors = new ReplaySubject(1);
    ...
    anyDataQueryLoading = new BehaviorSubject(false);
    ...
    dataQueriesByID = new Map();
    addDataQuery(id = DEFAULT_DATA_QUERY_ID) {
        if (this.published) {
            const err = new ConfigurationError(`DataQueries may only be added before the AppCore is published.`)
                .from(SOURCE)
                .at(Severity.FATAL);
            this.err(err);
            throw err;
        ...
        if (this.dataQueriesByID.has(id)) {
            const err = new ConfigurationError(`Multiple DataQueries cannot share the same ID '${id}'.`)
                .from(SOURCE)
                .at(Severity.FATAL);
            this.err(err);
            throw err;
    ...
    getDataQuery(id = DEFAULT_DATA_QUERY_ID) {
    ...
    publish() {
        ...
        combineLatest([...this.dataQueriesByID.values()].map((dq) => dq.loading))
            .pipe(map((loadingVals) => loadingVals.reduce((acc, current) => acc || current, false)), distinctUntilChanged())
            .subscribe((loading) => {
