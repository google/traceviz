...
function prettyprintCategory(rc, indent = '') {
    return [
        `${indent}Cat ${rc.category.id}`,
        `${indent}  x0:${rc.x0Px} y0:${rc.y0Px} x1:${rc.x1Px} y1:${rc.y1Px}`,
    ];
}
function prettyprintCategoryHierarchy(rch, indent = '') {
    const ret = [
        `${indent}Categories:`,
    ];
...
function prettyprintData(data, indent = '') {
    const ret = [];
    ...
    if (data instanceof RenderedBar) {
        ret.push(`${indent}Bar '${data.properties.expectString('label_format')}'`, `${indent}  x0:${data.x0Px} y0:${data.y0Px} x1:${data.x1Px} y1:${data.y1Px}`);
    }
...
function prettyprintBarChart(rbc, indent = '') {
    const ret = [
        `${indent}Bar chart`,
    ...
    ret.push(...prettyprintCategoryHierarchy(rbc.categoryHierarchy, indent + '  '));
