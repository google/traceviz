...
import { DataType, Keys } from './bar_chart.js';
...
class RenderedRect {
    ...
    x1Px;
    y1Px;
    constructor(x0Px, y0Px, x1Px, y1Px) {
        this.x0Px = x0Px;
        this.y0Px = y0Px;
        this.x1Px = x1Px;
        this.y1Px = y1Px;
...
export class RenderedBar extends RenderedRect {
    propertiesInternal;
    constructor(propertiesInternal, x0Px, y0Px, x1Px, y1Px) {
...
export class RenderedHorizontalBarChart {
    ...
    chartWidthPx;
    constructor(barChart, barChartWidthPx) {
        this.barChartWidthPx = barChartWidthPx;
        const categories = [];
        ...
        for (const cat of barChart.categories) {
                ...
                switch (dataType) {
                    case DataType.STACKED_BARS: {
                        ...
                        for (const child of barData.children) {
                            bars.push(new RenderedBar(child.properties, barChart.axis.valueToDomainFraction(child.properties, Keys.BAR_LOWER_EXTENT) *
                                this.chartWidthPx, y0, barChart.axis.valueToDomainFraction(child.properties, Keys.BAR_UPPER_EXTENT) *
                                this.chartWidthPx, y1));
                        }
                        this.data.push(new RenderedStackedBars(bars));
                        break;
                    }
                    case DataType.BAR: {
                        this.data.push(new RenderedBar(barData.properties, barChart.axis.valueToDomainFraction(barData.properties, Keys.BAR_LOWER_EXTENT) *
                            this.chartWidthPx, y0, barChart.axis.valueToDomainFraction(barData.properties, Keys.BAR_UPPER_EXTENT) *
                            this.chartWidthPx, y1));
                        break;
                    }
                    case DataType.BOX_PLOT: {
                        ...
                        const addQuantileFrom = (quantileFrom) => new RenderedBar(barData.properties, barChart.axis.valueToDomainFraction(barData.properties, quantileKeys[quantileFrom]) *
                            this.chartWidthPx, y0, barChart.axis.valueToDomainFraction(barData.properties, quantileKeys[quantileFrom + 1]) *
                            this.chartWidthPx, y1);
                        this.data.push(new RenderedBoxPlot(addQuantileFrom(0), addQuantileFrom(1), addQuantileFrom(2), addQuantileFrom(3)));
                        break;
                    }
                    default:
                        throw new ConfigurationError(`unsupported bar chart data type '${dataType}'`)
                            .from(SOURCE)
                            .at(Severity.ERROR);
                }
            ...
            const categoryHeight = Math.max(rs.categoryRenderSettings.categoryMinWidthCatPx, yOffsetFromStartOfCategory);
            categories.push(new RenderedCategory(cat.category, cat.properties, rs.categoryRenderSettings, 0, categoryYOffset, rs.categoryRenderSettings.categoryBaseWidthValPx, categoryYOffset + categoryHeight));
            categoryYOffset +=
