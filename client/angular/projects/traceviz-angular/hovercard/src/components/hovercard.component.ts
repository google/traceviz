...
export class HovercardComponent {
  ...
  private updatePositionBound = (evt: MouseEvent) => this.updatePosition(evt);
  ...
  ngOnChanges() {
    // react to visible toggles
    if (this.visible && !this.listening) {
      this.addListener();
    } else if (!this.visible && this.listening) {
      this.removeListener();
    }
  }
  ...
  private addListener() {
    window.addEventListener('mousemove', this.updatePositionBound);
    this.listening = true;
  }

  private removeListener() {
    if (this.listening) {
      window.removeEventListener('mousemove', this.updatePositionBound);
