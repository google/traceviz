...
import { AppCoreService, CoreModule, TestCoreModule } from "projects/ngx-traceviz-lib/public-api";
import { GLOBAL_TEST_DATA_FETCHER, ResponseNode, StringListValue, StringSetValue, StringValue, node, str, strs, valueMap } from "traceviz-client-core";
...
describe('update-values test', () => {
    ...
    it('updates from data series', () => {
        ...
        const uiFeatures = appCoreService.appCore.globalState.get('ui_features') as StringSetValue;
        const availableMetrics = appCoreService.appCore.globalState.get('available_metrics') as StringListValue;
        const defaultMetric = appCoreService.appCore.globalState.get('default_metric') as StringValue;
        GLOBAL_TEST_DATA_FETCHER.responseChannel.next({
            series: new Map<string, ResponseNode>([
                [tc.updateValuesDir.dataSeriesQueryDir!.dataSeriesQuery!.uniqueSeriesName,
                node(valueMap(
