...
import { AppCoreService, CoreModule, TestCoreModule } from 'projects/ngx-traceviz-lib/src/public-api';
import { GLOBAL_TEST_DATA_FETCHER, ResponseNode, StringValue, node, str, strs, valueMap } from 'traceviz-client-core';
import { DataTableModule } from './data-table.module';
import { NoopAnimationsModule } from '@angular/platform-browser/animations';

const tableData = node(
  valueMap(),
  node(valueMap(), // Column definitions
    node(valueMap(
...
describe('DataTableComponent', () => {
  ...
  beforeEach(async () => {
    TestBed.configureTestingModule({
      ...
      imports: [CoreModule, TestCoreModule, DataTableModule, NoopAnimationsModule],
      providers: [{
  ...
  it('should populate from backend', () => {
    ...
    GLOBAL_TEST_DATA_FETCHER.responseChannel.next({
      series: new Map<string, ResponseNode>([
        [tc.dataTableComp.dataSeriesQuery!.uniqueSeriesName,
          tableData],
      ]),
    });
    ...
    // Expect a header row.
    expect(Array.from(rows[0].querySelectorAll('th')).map((el) => el.innerText)).toEqual(['Name', 'Color', 'Flavor', 'Label']);
    // Expect three content rows.
    expect(Array.from(rows).slice(1).map(
      (rowEl) => Array.from(rowEl.querySelectorAll('.table-datum')).map(
        (el) => el.innerHTML.trim()))).toEqual([
    ...
    // Click some rows.
    const selectedFlavor = appCoreService.appCore.globalState.get('selected_flavor') as StringValue;
