...
class LiteralTestComponent {
  ...
  @ViewChild(StringLiteralListDirective)
  strLitListDir!: StringLiteralListDirective;
  @ViewChild(StringLiteralSetDirective)
  strLitSetDir!: StringLiteralSetDirective;
...
describe('value literal directive test', () => {
  ...
  it('handles literal Values', () => {
    ...
    expect((tc.strLitListDir.get(undefined) as StringListValue).val).toEqual([
      'a', 'b'
    ]);
    expect((tc.strLitSetDir.get(undefined) as StringSetValue).val)
        .toEqual(new Set(['c', 'd']));
    expect((tc.intLitDir.get(undefined) as IntegerValue).val).toEqual(3);
    expect((tc.intLitListDir.get(undefined) as IntegerListValue).val).toEqual([
      1, 2
    ]);
    expect((tc.intLitSetDir.get(undefined) as IntegerSetValue).val)
        .toEqual(new Set([3, 4]));
    expect((tc.dblLitDir.get(undefined) as DoubleValue).val).toEqual(3.14159);
    expect((tc.durLitDir.get(undefined) as DurationValue).val)
        .toEqual(new Duration(1000000));
    expect((tc.tsLitDir.get(undefined) as TimestampValue).val)
        .toEqual(new Timestamp(0, 0));
  });
});
