...
import {ConfigurationError, IntegerListValue, IntegerSetValue, Severity, StringListValue, StringSetValue, StringValue, Value, ValueMap} from 'traceviz-client-core';
...
export class Dropdown implements AfterContentInit, OnDestroy {
  ...
  ngAfterContentInit() {
    this.appCoreService.appCore.onPublish(() => {
      ...
      this.interactionsDirective?.get().watchAll(watchActions, this.unsubscribe)
          .subscribe((err) => {
            this.err(err);
          });
    });
